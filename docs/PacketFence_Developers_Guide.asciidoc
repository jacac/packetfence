...
----
# Debian
apt install -y build-essential libipset-dev pkg-config gettext
...
. Patch examples file (only necessary if running on Debian):
[source,bash]
----
git apply debian/patches/debianize.patch --verbose
----
